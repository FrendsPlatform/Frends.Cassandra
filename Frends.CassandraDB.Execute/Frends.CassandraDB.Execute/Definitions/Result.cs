...
public class Result
{
    ...
    /// <summary>
    /// Query results. JSON string of results (Only SELECT query will contain values).
    /// </summary>
    /// <example>{\"0\":[{\"Key\":\"foo\",\"Value\":\"bar\"}],\"1\":[{\"Key\":\"foo2\",\"Value\":\"bar2\"}]}</example>
    public string QueryResults { get; private set; }
    ...
    internal Result(bool success, string queryResults, List<string> warnings)
    {
        Success = success;
